...
int MPIDI_CH3I_RDMA_init(MPIDI_PG_t * pg, int pg_rank)
{
    ...
    /* Version one deliberately keeps one shared physical transport path per
     * process.  Reject accidental multi-rail configurations instead of
     * silently assigning the wrong remote SRQN. */
    if (rdma_num_hcas != 1 || rdma_num_ports != 1 ||
        rdma_num_qp_per_port != 1 || rdma_num_rails != 1) {
        MPL_error_printf("Hollow RC requires MV2_NUM_HCAS=1, one port, and MV2_NUM_QP_PER_PORT=1\n");
        mpi_errno = MPI_ERR_OTHER;
        goto fn_fail;
    }
    if (!mv2_MPIDI_CH3I_RDMA_Process.has_srq) {
        MPL_error_printf("Hollow RC requires the MVAPICH2 SRQ receive path\n");
        mpi_errno = MPI_ERR_OTHER;
        goto fn_fail;
    }
    for (i = 0; i < MAX_NUM_HCAS; i++)
        mv2_MPIDI_CH3I_RDMA_Process.hollow_xrcd_fd[i] = -1;
    mv2_MPIDI_CH3I_RDMA_Process.has_ring_startup = 0;
#ifdef RDMA_CM
    ...
    if (pg_size > 1) {
        ...
        if (!mv2_MPIDI_CH3I_RDMA_Process.has_ring_startup) {
            ...
            for (i = 0; i < pg_size; i++) {
                ...
                sprintf(buf, "%08X", MV2_ACTIVE_TRANSPORT_ABI);
                buf += 8;
                ...
                for (rail_index = 0; rail_index < rdma_num_rails;
                     rail_index++) {
                    sprintf(buf, "%08X",
                            init_info->hollow_srqn[i][rail_index]);
                    buf += 8;
                }
#endif
            ...
            for (i = 0; i < pg_size; i++) {
                ...
                {
                    unsigned int transport_abi = 0;
                    if (sscanf(buf, "%08X", &transport_abi) != 1 ||
                        transport_abi != MV2_ACTIVE_TRANSPORT_ABI) {
                        MPL_error_printf("mrail startup: rank %d uses an incompatible RC transport mode/ABI\n", i);
                        mpi_errno = MPI_ERR_OTHER;
                        goto fn_fail;
                    }
                    buf += 8;
                ...
                for (rail_index = 0; rail_index < rdma_num_rails;
                     rail_index++) {
                    if (sscanf(buf, "%08X",
                               &init_info->hollow_srqn[i][rail_index]) != 1) {
                        MPL_error_printf("Hollow RC startup: rank %d did not publish an XRC SRQN\n", i);
                        mpi_errno = MPI_ERR_OTHER;
                        goto fn_fail;
                    }
                    buf += 8;
...
#ifdef _ENABLE_HOLLOW_RC_
            if (mv2_MPIDI_CH3I_RDMA_Process.hollow_xrcd[i]) {
                ibv_close_xrcd(
                    mv2_MPIDI_CH3I_RDMA_Process.hollow_xrcd[i]);
                mv2_MPIDI_CH3I_RDMA_Process.hollow_xrcd[i] = NULL;
            }
            if (mv2_MPIDI_CH3I_RDMA_Process.hollow_xrcd_fd[i] >= 0) {
                close(mv2_MPIDI_CH3I_RDMA_Process.hollow_xrcd_fd[i]);
                mv2_MPIDI_CH3I_RDMA_Process.hollow_xrcd_fd[i] = -1;
            }
#endif
        }
...
#ifdef _ENABLE_HOLLOW_RC_
    if (mv2_MPIDI_CH3I_RDMA_Process.hollow_retry_set != NULL) {
        MPIU_Free(mv2_MPIDI_CH3I_RDMA_Process.hollow_retry_set);
        mv2_MPIDI_CH3I_RDMA_Process.hollow_retry_set = NULL;
        mv2_MPIDI_CH3I_RDMA_Process.hollow_retry_capacity = 0;
