...
#define MV2_NORMAL_RC_TRANSPORT_ABI 0x52430001U
#define MV2_HOLLOW_RC_TRANSPORT_ABI 0x48520001U
#ifdef _ENABLE_HOLLOW_RC_
#define MV2_ACTIVE_TRANSPORT_ABI MV2_HOLLOW_RC_TRANSPORT_ABI
#else
#define MV2_ACTIVE_TRANSPORT_ABI MV2_NORMAL_RC_TRANSPORT_ABI
#endif
...
#ifdef _ENABLE_HOLLOW_RC_
    /* Shared receive namespace used by the Hollow RC logical QPs. */
    struct ibv_xrcd             *hollow_xrcd[MAX_NUM_HCAS];
    int                          hollow_xrcd_fd[MAX_NUM_HCAS];
    uint32_t                     hollow_srqn[MAX_NUM_HCAS];
    volatile int                 hollow_blocked_rails;
    int                          hollow_retry_capacity;
    MPIDI_VC_t                 **hollow_retry_set;
#endif
...
do {                                                                   \
    int __ret;                                                         \
    struct ibv_send_wr *__head = &((_v)->desc.u.sr);                   \
    XRC_FILL_SRQN_FIX_CONN((_v), (_c), (_rail));                       \
    MPIU_Assert((_rail) == (_v)->rail);                                \
    MPIU_Assert((_c)->mrail.rails[(_rail)].hollow_remote_srqn != 0);   \
    mv2_MPIDI_CH3I_RDMA_Process.global_used_send_cq++;                 \
    __ret = ibv_post_send((_c)->mrail.rails[(_rail)].qp_hndl,          \
                          __head, &((_v)->desc.y.bad_sr));              \
    if (unlikely(__ret)) {                                             \
        mv2_MPIDI_CH3I_RDMA_Process.global_used_send_cq--;             \
        if ((__ret == ENOMEM || __ret == EAGAIN) &&                    \
            (_v)->desc.y.bad_sr == __head) {                           \
            ++(_c)->mrail.rails[(_rail)].send_wqes_avail;             \
            if (!(_c)->mrail.rails[(_rail)].hollow_sq_blocked) {       \
                MPIU_Assert((_c)->pg_rank >= 0 &&                     \
                            (_c)->pg_rank <                            \
                            mv2_MPIDI_CH3I_RDMA_Process.               \
                                hollow_retry_capacity);                \
                (_c)->mrail.rails[(_rail)].hollow_sq_blocked = 1;      \
                __atomic_store_n(                                     \
                    &mv2_MPIDI_CH3I_RDMA_Process.                      \
                         hollow_retry_set[(_c)->pg_rank],              \
                    (_c), __ATOMIC_RELEASE);                           \
                __sync_fetch_and_add(                                  \
                    &mv2_MPIDI_CH3I_RDMA_Process.hollow_blocked_rails, \
                    1);                                                \
            }                                                          \
            MRAILI_Hollow_enqueue_retry((_c), (_rail), (_v));         \
        } else {                                                       \
            fprintf(stderr,                                           \
                    "Hollow RC post_send failed: ret=%d errno=%d "    \
                    "rank=%d rail=%d bad_wr=%p head=%p\n",            \
                    __ret, errno, (_c)->pg_rank, (_rail),              \
                    (void *)(_v)->desc.y.bad_sr, (void *)__head);      \
...
#endif /* transport-specific post path */
...
void MRAILI_Hollow_enqueue_retry(MPIDI_VC_t *vc, int rail, vbuf *v);
void MRAILI_Hollow_retry_blocked(void);
